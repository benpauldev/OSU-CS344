...
int main(int argc, char *argv[])
{
    ...
    char *key_buffer = malloc(sizeof(char*) * 100000);          //create a buffer for key text
    
    getTextEncode(key_buffer, argv[2]);                             //get text from key hold in key_buffer
    
    char *file_buffer = malloc(sizeof(char*) * 100000);         //create a buffer for file text
    
    getTextEncode(file_buffer, argv[1]);                            //get text from filehold in file_buffer
    
    if (strlen(key_buffer) < strlen(file_buffer))           //if the key is shorter than the file to encode the key is too short
    {
    ...
    if (test < 0)                                                  //error if send key failed
    {
    ...
    while((test = read(sockfd,char_buffer,255)) > 0)               //while reciving from socket
    {
        if (test < 0)
        {
            error("Error receiving from socket");                 //error receiving from socket
        }
